...
class WelchAlgorithm :
	...
	def launch(self, arrival_rate, nb_customers, service_rate, no_of_servers, capacity):
		...
		for i in range(self.simulation_nb):
			...
			total_measure.append([measures[time].queue_length for time in [measure for measure in measures]])
		...
		for i in range(len(meanMeasure)):
			if (i < self.window):
				welchMeasure[i] = float(sum(meanMeasure[i-(i) : i+(i)])) / max(len(meanMeasure[i-(i) : i+(i)]), 1)
